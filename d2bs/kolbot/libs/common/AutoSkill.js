	...
	*/
	...
	this.skillToAdd = function (inputArray) {
		for (var i = 0; i < inputArray.length; i += 1) {
			...
			if (inputArray[i][1] > 20) {
				print("ÿc4AutoSkill ÿc0:: ÿc9Skill build index " + i + " ÿc9has allocation count of " + inputArray[i][1] + " ÿc9and it will be limited to 20.");
				inputArray[i][1] = 20;
			}
	...
	this.allocate = function () {
		...
		if (!getUIFlag(0x17)) {
			...
			if (addTo) {
				print("ÿc4AutoSkill ÿc0:: ÿc2Using skill point in Skill ID ÿc0:: ÿc4 " + addTo);
				delay(100);
	...
	this.init = function (skillBuildOrder, save = 0) {
		...
		if (!this.skillBuildOrder || !this.skillBuildOrder.length) {
			print("ÿc4AutoSkill ÿc0:: ÿc1No build array specified!");
		...
		print("ÿc4AutoSkill ÿc0:: ÿc2Finished allocating skill points!");
