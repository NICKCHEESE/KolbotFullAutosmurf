...
MainLoop:
			...
			while (getTickCount() - tick < 1000) {
				...
				if (stats.classid === 523) {
					if (!item.getStat(14) || item.getStat(14) < stats.gold) {
						print("ÿc4Pickit ÿc0:: ÿc4Picked up ÿc0:: ÿc< " + stats.color + (item.getStat(14) ? (item.getStat(14) - stats.gold) : stats.gold) + " " + stats.name);
	...
	canPick: function (unit) {
		...
		case undefined: // Yes, it does happen
			print("ÿc4Pickit ÿc0:: ÿc8Undefined item (!?)");
