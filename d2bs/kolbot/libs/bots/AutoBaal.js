		...
		do {
			...
			do {
				...
				if (suspect.area === destination && !getPlayerFlag(me.gid, suspect.gid, 8)) { // first player not hostile found in destination area...
					...
					print("ÿc4AutoBaal ÿc0:: ÿc0Autodetected leader ÿc0:: ÿc; " + leader);
					return true;
				}
	...
	if (leader || autoLeaderDetect(131)) { // find the first player in area 131 - throne of destruction
		while (Misc.inMyParty(leader)) { // do our stuff while partied
			...
			if (throneCheck && me.area === 109) { // wait for throne signal - leader's safe message
				print("ÿc4AutoBaal ÿc0:: ÿc3Trying to take TP to throne.");
			...
			if (baalCheck && me.area === 131) { // wait for baal signal - leader's baal message
				...
				print("ÿc4AutoBaal ÿc0:: ÿc3Entering chamber.");
